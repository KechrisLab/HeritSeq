...
Alternatively, users may download an alternative functions *fit.CP_alt( )* 
directly from the GitHub Page and directly fit multiple features.

```{r, eval = FALSE}
source("fit.CP_alt.R")
result.cp <- fit.CP_alt(CountMatrix = simData, Strains = strains, test = FALSE, optimizer = "nlminb")
vpc.cp <- computeVPC.CP(para = result.cp[[1]])
```
...
caution. To avoid this issue, user may source an alternative functions 
*fit.CP_alt( )* and *getBootCI_alt( )* from the GitHub page and compute the CIs
as following: 

Compute CI based on 100 boostrap samples for the first 3 features using the CP
method:
```{r, eval = FALSE}
source(fit.CP_alt.R)
source(getBootCI_alt.R)
boot.vst <- getBootCI_alt(CountMatrix = simData, 
                      Strains = strains, 
                      which.features = 1:3, 
                      num.boot = 100, 
                      method = "CP",
