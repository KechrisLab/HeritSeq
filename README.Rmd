...
```{r, eval = FALSE}
result.cp <- fit.CP(CountMatrix = simData, Strains = strains, test = FALSE, optimizer = "nlminb")
vpc.cp <- computeVPC.CP(para = result.cp[[1]])
```
Similar to *fit.NB( )*, the function *fit.CP( )* returns a list of two objects. The first object consists the fitted parameters $\alpha_{g}, \sigma^{2}_{g}, p_{g}, \phi_{g}$. The second object provides the p-value for testing the presence of heritability if test = TRUE; it returns NULL otherwise (default).
...
## Hypothesis testing
As illustrated in the examples above, each of the four methods above also allows hypothesis testing for presence of heritability, simply by setting Test = TRUE in the functions *fit.NB( )*, *fit.CP( )*, and *fitComputeVPC.lmer( )* for NBMM, CPMM, and LMM respectively. 
